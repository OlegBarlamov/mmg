    ...
    public static class GalaxyTextureGenerator
    {
            ...
            float armAngleOffset = 0f)
        {
            ...
            try
            {
                ...
                RenderStars(brightStars, 0f, 0f, diskRadius, galaxyColor, colors, textureSize, armAngleOffset);
            ...
            try
            {
                ...
                foreach (var sector in sectors)
                {
                    ...
                    RenderStars(sector.ClusterPoints, sector.CenterX, sector.CenterZ,
                        diskRadius, galaxyColor, colors, textureSize, armAngleOffset);
                }
                cancellationToken.ThrowIfCancellationRequested();
                RenderBulge(colors, textureSize, galaxyColor);
                cancellationToken.ThrowIfCancellationRequested();
        ...
        private static void RenderStars(
            ...
            Color[,] colors, int textureSize, float armAngleOffset)
        {
            var center = textureSize / 2f;
            var scale = center / diskRadius;
            var galaxyTint = galaxyColor.ToVector3();
            var cosOff = (float)Math.Cos(armAngleOffset);
            var sinOff = (float)Math.Sin(armAngleOffset);

            for (int i = 0; i < stars.Length; i++)
            {
                var star = stars[i];
                var worldX = star.X + offsetX;
                var worldZ = star.Z + offsetZ;

                var sx = worldX;
                var sz = worldZ;
                if (armAngleOffset != 0f)
                {
                    var rx = sx * cosOff - sz * sinOff;
                    var rz = sx * sinOff + sz * cosOff;
                    sx = rx;
                    sz = rz;
                }
                ...
                var distFromCenter = (float)Math.Sqrt(worldX * worldX + worldZ * worldZ) / (diskRadius * 0.9f);
